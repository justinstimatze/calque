...
def score_pair(a: FuncSig, b: FuncSig) -> Suspicion | None:
    name = 1.0 if a.stem and a.stem == b.stem else _jaccard(a.stem, b.stem)
    sig = {
    ...
    # Gate out noise: require at least a real role-overlap OR a concrete
    # surface/effect overlap. A pair that only shares generic call names is junk.
    has_anchor = (
        name >= 0.34 or sig["strings"] > 0 or sig["writes"] > 0 or sig["ret"] > 0
    )
    if not has_anchor:
        return None
