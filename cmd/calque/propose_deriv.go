...
func runProposeDeriv(args []string) {
	...
	includeTests := fs.Bool("include-tests", false, "pair test↔test derivations too (gated by default — two test cases sharing a setup/mock read-set are the read-set's degenerate false twin; a test↔prod derivation, a test recomputing a production quantity, is always kept)")
	...
	// Test files are NOT excluded wholesale — the asymmetric gate in
	// SharedDerivationCandidates drops only test↔test pairs, keeping test↔prod
	// (a test recomputing a production quantity is real drift, not fixture noise).
	...
	cands := code.SharedDerivationCandidates(sigs, *minReads, *readJac, *maxFanout, *includeMutators, *includeStructural, *includeTests)
